...
class CopulaDecoder(nn.Module):
    ...
    def __init__(
        ...
        skip_copula=False,
        t_copula: Optional[Dict[str, Any]] = None,
        gaussian_copula: Optional[Dict[str, Any]] = None,
    ):
        """
        Parameters:
        -----------
        ...
        t_copula: Dict[str, Any], default to None
            If set to a non-None value, uses a TCopula.
            The options sent to the TCopula is content of this dictionary.
        gaussian_copula: Dict[str, Any], default to None
            If set to a non-None value, uses a GaussianCopula.
            The options sent to the GaussianCopula is content of this dictionary.
        """
        super().__init__()
        ...
        self.t_copula_args = t_copula
        self.gaussian_copula_args = gaussian_copula

        if not self.skip_copula:
            if attentional_copula is not None:
                self.copula = AttentionalCopula(**attentional_copula)
            elif t_copula is not None:
                self.copula = TCopula(**t_copula)
            elif gaussian_copula is not None:
                self.copula = GaussianCopula(**gaussian_copula)
...
class TCopula(nn.Module):
    """
    A t-copula based on attention between the various variables.
    """
...
class GaussianCopula(nn.Module):
    """
    A Gaussian copula based on attention between the various variables.
    """
